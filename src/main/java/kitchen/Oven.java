	...
	public void bake(Batch batch) {
		...
		for (Pastry pastry : batch.getBatch()) {
			...
			timer.schedule(new TimerTask() {
				@Override
				public void run() {
					LOG.debug("batch finished. it is " + pastry.cookedStatus + ". It took " + rn + " milleseconds"
							+ "total weight was " + pastry.weight);
				}
			}, rn);

			File file = new File("C:\\temp\\kitchenInputOutput\\output\\" + pastry.type + w + ".txt");
			try {
				file.createNewFile();
			} catch (IOException e) {
				e.printStackTrace();
